...
import com.github.prontera.model.CoordinatorParticipant;
...
public class CoordinateService {
    ...
    public void confirm(TccRequest request) {
        ...
        final List<CoordinatorParticipant> participantLinks = request.getParticipantLinks();
        Preconditions.checkNotNull(participantLinks);
        //checkExpireInLocal(request, participantLinks);
        // 调用确认资源链接
        int success = 0;
        int fail = 0;
        for (CoordinatorParticipant participant : participantLinks) {
    ...
    public void cancel(TccRequest request) {
        Preconditions.checkNotNull(request);
        final List<CoordinatorParticipant> participantList = Preconditions.checkNotNull(request.getParticipantLinks());
        try {
            for (CoordinatorParticipant participant : participantList) {
