...
public final class Responses {
    ...
    public static <T extends ResolvableResponse> T generate(@Nonnull Class<T> clazz, @Nonnull StatusCode status) {
        return generate(clazz, status, status.message());
    }

    public static <T extends ResolvableResponse> T generate(@Nonnull Class<T> clazz, @Nonnull StatusCode status, @Nonnull Object message) {
        Objects.requireNonNull(clazz);
        Objects.requireNonNull(status);
        Objects.requireNonNull(message);
        T instance = OBJENESIS.newInstance(clazz);
        instance.setSuccessful(StatusCode.isSuccess(status));
    ...
    public static <T extends ResolvableResponse> T generate(@Nonnull Class<T> clazz, @Nonnull Throwable throwable) {
        ...
        if (exception instanceof ResolvableStatusException) {
            statusCode = ((ResolvableStatusException) exception).getStatusCode();
